...
export default function TradingCards({ productData }) {
    ...
    React.useEffect(() => {
        ...
        const filteredProducts = Array.isArray(productData) ?
            productData.filter(product => product.category === "Trading-Cards") :
        ...
        <Container maxWidth="xl" sx={{ mt: 2 }}>
            ...
            <Box display={"flex"} flexDirection={"row"} flexWrap={"wrap"} width={"100%"} sx={{ gap: 2 }} mt={2} mb={2} alignItems={"center"} justifyContent={"center"} >
                {productDetails.map((product, index) =>
                    <Box key={index} width={{ xs: "100%", md: "20%" }} margin={"auto"} component={"a"} sx={{ textDecoration: "none" }} href={`trading-cards/${product.id}`}>
